...
def _my_top_k(x, k):
  ...
  for i in range(k):
    ...
    if i + 1 < k:
      x += tf.one_hot(argmax, depth, -1e9) # adding negative infinity to the small values
...
def noisy_top_k_gating(x,
  ...
  with tf.variable_scope(name, default_name="noisy_top_k_gating"):
    ...
    w_gate = tf.get_variable(
        "w_gate", [input_size, num_experts], tf.float32, initializer) # Artur: how does it retrieve the w_gate
    if noisy_gating:
      w_noise = tf.get_variable("w_noise",
                                [input_size, num_experts], tf.float32,
                                initializer)
    #clean_logits = tf.matmul(x, w_gate) # Artur --> here we pass x through the FFN, to produce the vector of normal dist
    # values, the FFN should output a vector with a dimension of the number of experts, the values of the vector will
    # be the Gaussian probabilities

    # for now not training end to end (similar to the autoencoders paper, the GMM model would have been pre-trained
    # in advance, hence in here when computing the clean_logits, just need to plug the output of the
    # FFNN that generates the GMM output

    clean_logits = gaussian_clean_logits()
    ...
    top_k_gates = tf.nn.softmax(top_k_logits) # Artur: here instead of running softmax
